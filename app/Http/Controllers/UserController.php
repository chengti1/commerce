...
class UserController extends BaseController
{
    ...
    public function getUpdateProfile(){
        session()->regenerate();
        if(session('usertype') == 'pending'){
            $userData = User::where('user_id', session('user_id'))->first();
            return view('update-profile')->with('userData',$userData);
        }
    }

    public function postUpdateProfile(UpdateProfileRequest $request){
        session()->regenerate();
        User::where('user_id', session('user_id'))->update(['email' => $request->input('email'), 'first_name' => $request->input('fname'), 'last_name' => $request->input('lname'), 'address1' => $request->input('add1'), 'address2' => $request->input('add2'), 'Company' => $request->input('company'), 'region' => $request->input('region'), 'postal_code' => $request->input('pcode'), 'country' => $request->input('country'), 'paypal_email' => $request->input('paypalemail')]);
        $data = array('email'=>$request->input('email'), 'name'=>$request->input('fname'));
        User::where('email',$request->input('email'))->update(['created_by'=>session('user_id'), 'updated_by' => session('user_id')]);
        $code = User::where('email', $request->input('email') )->first();
        Mail::send('mails.welcome', ['confirm_code'=>$code->confirm_code, 'user_id'=>$code->user_id], function($message) use($data)
        {
            $message->from('testmail0987654@gmail.com', 'Bubiland');
